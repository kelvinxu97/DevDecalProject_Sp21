...
contract MerchToken is ERC721, Ownable {
    ...
    function changePrice(uint256 _tokenId, uint256 _newPrice) public {
        require(_newPrice != 0); //require a non-zero new price
        require(_tokenId < index); //require the tokenId to exist
        require(ownerOf(_tokenId) == msg.sender); //require function caller to be the token owner
        tokenInfos[_tokenId].price = _newPrice; //change the price
    }
